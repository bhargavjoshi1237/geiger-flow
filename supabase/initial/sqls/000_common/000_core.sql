...
using (
  created_by = auth.uid()
  or public.flow_is_project_member(id)
);
...
using (public.flow_is_project_member(id))
with check (public.flow_is_project_member(id));
...
for select to authenticated
using (project_id is null or public.flow_is_project_member(project_id));
