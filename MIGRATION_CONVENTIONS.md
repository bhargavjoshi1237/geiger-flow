...
> **This product's schema is `flow`.** Its migration ledger is
> `flow.geiger_migrations`. Substitute `flow` wherever this doc writes
...
(`flow.tickets`), never `flow.flow_tickets`.
...
-- Add seat maps
--
-- Owns flow.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists flow.seat_maps (
...
-- @down
drop table if exists flow.seat_maps;
```
...
create table if not exists flow.tickets (...);
alter table flow.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on flow.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on flow.tickets;
create trigger tickets_touch_updated_at
before update on flow.tickets
for each row execute function flow.touch_updated_at();

drop policy if exists tickets_demo_all on flow.tickets;
create policy tickets_demo_all on flow.tickets for all to anon, authenticated
...
```sql
alter table flow.tickets enable row level security;
drop policy if exists tickets_demo_all on flow.tickets;
create policy tickets_demo_all on flow.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on flow.tickets (event_id);
```
